...
<div class="table-responsive">
    ...
    if (count($rows->lists['players'])) {
        ?>
    <form role="form" method="post" lpformnum="1" class="players-table-form">
    <table class="table table-striped cansorttbl players-table-resp" id="jstable_plz">
        <thead>
            <tr>
        ...
        ?>
                ...
                <?php 
                    if (isset($lists['pagination']) && $lists['pagination']) {
                        ?>
                    <a href="<?php echo classJsportLink::playerlist($rows->season_id, '&sortf=first_name&sortd='.$dest.'&playerevents='.$rows->playerevents)?>"><span><?php echo classJsportLanguage::get('BLFA_NAME');
                        ?></span><i class="fa"></i></a>
        ...
        ?>
                ...
                if (isset($rows->lists['played_matches_col']) && $rows->lists['played_matches_col']) {
                    ...
                    <th class="jsTextAlignCenter <?php echo $class?>">
                        ...
                        if (isset($lists['pagination']) && $lists['pagination']) {
                            ?>
                        <a href="<?php echo classJsportLink::playerlist($rows->season_id, '&sortf=played&sortd='.$dest.'&playerevents='.$rows->playerevents)?>"><span><?php echo $rows->lists['played_matches_col'];
                            ?></span><i class="fa"></i></a>
            ...
            foreach ($rows->lists['events_col'] as $key => $value) {
                ...
                ?>
                        <th class="jsTextAlignCenter <?php echo $class?>">
                            ...
                            if (isset($lists['pagination']) && $lists['pagination']) {
                                ?>
                            <a href="<?php echo classJsportLink::playerlist($rows->season_id, '&sortf='.$key.'&sortd='.$dest.'&playerevents='.$rows->playerevents)?>">
                                <span>
